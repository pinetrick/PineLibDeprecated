...
public class OpenFileBySys
{
	private static G g = new G(OpenFileBySys.class);



	public static void open(File currentPath)
	{
		g.d("打开文件" + currentPath.toString());
		if (currentPath != null && currentPath.isFile())
		{
			String fileName = currentPath.toString();
			Intent intent;
			if (checkEndsWithInStringArray(fileName, A.a().getResources()
					.getStringArray(R.array.fileEndingImage)))
			{
				intent = getImageFileIntent(currentPath);
				A.a().startActivity(intent);
			}
			else if (checkEndsWithInStringArray(fileName, A.a().getResources()
			...
			{
				MessageBox.i().show("无法打开该文件！");
			}
		}
		else
		{
			MessageBox.i().show("对不起，这不是文件！");
